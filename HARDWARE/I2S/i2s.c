...
{  
	...
	I2S_Cmd(I2S3ext,ENABLE);		//I2S3ext I2S EN使能.
...
{  
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1,ENABLE);//DMA1时钟使能 
	...
	while (DMA_GetCmdStatus(DMA1_Stream7) != DISABLE){}//等待DMA1_Stream7可配置 
		
	DMA_ClearITPendingBit(DMA1_Stream7,DMA_IT_FEIF7|DMA_IT_DMEIF7|DMA_IT_TEIF7|DMA_IT_HTIF7|DMA_IT_TCIF7);//清空DMA1_Stream7上所有中断标志
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (u32)&i2splaybuf1[0] ;//DMA 存储器0地址
  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;//存储器到外设模式
  DMA_InitStructure.DMA_BufferSize = DMA_BUF_SIZE/2;//数据传输量 
  ...
  DMA_Init(DMA1_Stream7, &DMA_InitStructure);//初始化DMA Stream
	...
	DMA_DoubleBufferModeConfig(DMA1_Stream7,(u32)&i2splaybuf2[0],DMA_Memory_0);//双缓冲模式配置
...
{ 	
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_DMA1,ENABLE);//DMA1时钟使能 
	...
	while (DMA_GetCmdStatus(DMA1_Stream0) != DISABLE){}//等待DMA1_Stream0可配置 
		
	DMA_ClearITPendingBit(DMA1_Stream0,DMA_IT_FEIF0|DMA_IT_DMEIF0|DMA_IT_TEIF0|DMA_IT_HTIF0|DMA_IT_TCIF0);//清空DMA1_Stream0上所有中断标志

  /* 配置 DMA Stream */

  DMA_InitStructure.DMA_Channel = DMA_Channel_3;  //通道2 I2S3ext_RX通道 
  DMA_InitStructure.DMA_PeripheralBaseAddr = (u32)&I2S3ext->DR;//外设地址为:(u32)&I2S2ext->DR>DR
  DMA_InitStructure.DMA_Memory0BaseAddr = (u32)&i2srecbuf1[0];//DMA 存储器0地址
  ...
  DMA_Init(DMA1_Stream0, &DMA_InitStructure);//初始化DMA Stream
	...
	DMA_DoubleBufferModeConfig(DMA1_Stream0,(u32)&i2srecbuf2[0],DMA_Memory_0);//双缓冲模式配置
...
{      
	if(DMA_GetITStatus(DMA1_Stream7,DMA_IT_TCIF7)==SET)////DMA1_Stream7,传输完成标志
	{ 
...
{      
	if(DMA_GetITStatus(DMA1_Stream0,DMA_IT_TCIF0)==SET)	//DMA1_Stream0,传输完成标志
	{ 
